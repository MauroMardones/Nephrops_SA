...
bac_tab <- bac %>%
  transmute(
    ...
    CPUE = LPUE_std         # Standardised CPUE
  )
kable(
  bac_tab,
  format = "latex",
...
scenarios_long2 <- data.frame(
  ...
  Scenario_description = c(
    ...
    rep(
      "Scenario combines total landings with the ISUNEPCA UWTV biomass index, ARSA yield, and the standardised commercial LPUE, integrating long-term and recent information while ensuring consistency among data sources.",
      4
    )
  ),
...
The following sections present a synthesis of the key findings from these new scenarios, including diagnostic evaluations, stock dynamics, and management implications, while also referencing relevant results from the previously documented scenarios (`SC0` to `SC5`) for comparative purposes.
...
```{r brps, fig.cap="Comparison of stochastic biological reference points (Bmsy, Fmsy, MSY) and current catch (Cp) across SPiCT Scenarios SC5–SC9 and prior configurations (RUN5–RUN8) for Nephrops FU30. Points represent scenario–prior combinations, illustrating the sensitivity of stock productivity and exploitation reference points to alternative data configurations and prior assumptions.", out.width="\\textwidth"}
knitr::include_graphics("../figs/SPiCT_BRP_scenario_5_8_RUN_comparison.png")
```


```{r plotspictsc5r5, fig.cap="Plot Spict Scenario 5 RUN 5", out.width="\\textwidth"}
knitr::include_graphics("../figs/spict_plots/SPICT_SC5_RUN5.png")
```

```{r plotspictsc9r8, fig.cap="Plot Spict Scenario 9 RUN 8", out.width="\\textwidth"}
...
kable(
  ...
  format = "latex",
  booktabs = TRUE,
  caption = "Stochastic biological reference points and estimated current catch (Cp) by SPiCT scenario and prior configuration (RUN) for Nephrops FU30.",
  col.names = c("Scenario", "Run", "$B_{MSY}$", "$F_{MSY}$", "MSY", "Cp"),
  align = "lccccc"
) %>%
  kable_styling(
    latex_options = c("repeat_header", "scale_down"),
