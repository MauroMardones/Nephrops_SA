...
To extend the SPiCT-based assessment of *Nephrops*, we defined **five additional modelling scenarios (Scenarios 5–9)** that incorporate alternative combinations of fishery-dependent and fishery-independent information. *Scenario 5* combines total landings with the **normalized ISUNEPCA UWTV biomass index** and the **normalized ARSA biomass index**, providing a configuration based exclusively on survey-derived biomass information. *Scenario 6* integrates total landings with the **normalized ISUNEPCA UWTV biomass index** and a **standardised LPUE**, allowing direct comparison between fishery-independent biomass and fishery-dependent abundance signals. *Scenario 7* represents a more information-rich configuration that combines **normalized ISUNEPCA biomass**, **normalized ARSA biomass**, and **standardised LPUE** together with landings. *Scenario 8* combines landings with the **normalized ISUNEPCA UWTV abundance index** and **standardised LPUE**, providing an alternative representation of stock dynamics based on abundance rather than biomass. The definition of *Scenario 9* is motivated by a major structural change in the ARSA survey. In 2012, the ARSA survey programme experienced a change of vessel and operational characteristics, introducing a potential discontinuity in the biomass index methodology estimation. To avoid confounding biological signals with survey artefacts, only the pre-2012 ARSA biomass series is retained in this scenario. Post-2012 stock dynamics are instead informed by the ISUNEPCA UWTV survey, which provides a consistent biomass index from 2015 onward, and by the standardised LPUE, which reflects commercial fleet performance in recent years. This scenario therefore provides a coherent data configuration that respects survey continuity while preserving both long-term and contemporary information for stock assessment (Table \@ref(tab:newscenario)). All scenarios include total landings as the catch time series and differ in the configuration of abundance indices (Figure \@ref(fig:indices)).
...
scenarios_df1 <- data.frame(
  ...
  Input_Data = c(
    ...
    "Catches; ISUNEPCA UWTV abundance (normalized); Standardised LPUE",
    "CATCHES; ISUNEPCA UWTV biomass (normalized), ARSA biomass (normalized), Standardised LPUE"
  ),
  ...
  Description = c(
   ...
   "Scenario integrating total landings with normalized ISUNEPCA UWTV abundance and a standardized LPUE index, combining fishery-independent and fishery-dependent information.",
   "Scenario combines total landings with the ISUNEPCA UWTV biomass index (2015–2025), ARSA biomass survey (1993–2012), and the standardised commercial LPUE (2009–2024). This configuration integrates long-term and recent information while ensuring consistency among data sources."
