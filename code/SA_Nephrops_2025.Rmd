...
```{r spictscenarios, echo=FALSE, message=FALSE, results='asis'}
tab_scenarios <- data.frame(
  Scenario = c("Scenario 1", "Scenario 2", "Scenario 3", "Scenario 4"),
  
  Input_Data = c(
    "Catches (1994–2023); ARSA spring index; ARSA autumn index; ISUNEPCA UWTV biomass index; ISUNEPCA UWTV abundance index; ARSA spring productivity index; Commercial LPUE",
    "Catches (1994–2023); ISUNEPCA UWTV biomass index; Commercial LPUE",
    "Catches (1994–2023); ARSA spring index; ISUNEPCA UWTV biomass index (ISUNEPBIO2)",
    "Catches (1994–2023); ISUNEPCA UWTV biomass index"
  ),
  ...
  Description_Priors = c(
    "Baseline configuration using all available survey and fishery-dependent indices. Default SPiCT priors applied to evaluate model behaviour under maximum data availability.",
    "Reduced configuration focusing on the UWTV biomass index combined with commercial LPUE to assess robustness under limited survey information.",
    "Intermediate configuration combining the long ARSA spring time series with the UWTV biomass index to evaluate consistency between fishery-independent surveys.",
    "Minimal data configuration using only catches and the UWTV biomass index, representing a conservative assessment approach for data-limited stocks."
  ),
  
  stringsAsFactors = FALSE
)

ft_scenarios <- flextable(tab_scenarios) |>
  set_header_labels(
    Scenario = "Scenario",
    Input_Data = "Input data",
    Priors = "Priors",
    Description_Priors = "Description of priors"
  ) |>
  ...
  width(j = 3, width = 2) |>
  width(j = 4, width = 3) |>
  set_caption("Summary of SPiCT scenarios and input data used for Nephrops FU30.")
